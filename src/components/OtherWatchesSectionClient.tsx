...
export default function OtherWatchesSectionClient({ products }: { products: Product[] }) {
    ...
    return (
        <section id="other-watches" className="py-20 relative z-10 bg-slate-50 border-t border-slate-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                    <div className="max-w-2xl">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-100/50 text-orange-700 text-sm font-bold mb-4 border border-orange-200">
                            <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />
                            ساعات ذكية
                        </div>
                        <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mb-4 leading-tight">
                            ساعات <span className="text-orange-600">الشركات الأخرى</span>
                        </h2>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {products.map((product, index) => (
                        <motion.div
                            ...
                            whileHover={{ y: -5 }}
                            className="bg-white rounded-3xl p-5 relative group overflow-hidden border border-slate-200 hover:shadow-xl hover:border-orange-300 transition-all flex flex-col h-full"
                        >
                            {product.tag && (
                                <div className="absolute top-4 right-4 z-10 px-2 py-1.5 rounded-md bg-white/90 backdrop-blur-sm text-orange-700 text-xs font-bold shadow-sm border border-slate-100">
                                    {product.tag}
                                </div>
                            )}
                            ...
                            <div className={`w-full aspect-square rounded-2xl ${product.bg_color || 'bg-slate-100'} mb-6 flex flex-col items-center justify-center relative overflow-hidden group-hover:scale-[1.02] transition-transform duration-500`}>
                                <div className="w-1/2 h-1/2 rounded-full border-4 border-slate-800 bg-black flex items-center justify-center shadow-lg relative">
                            ...
                            <div className="flex-1 flex flex-col">
                                ...
                                <div className="pt-4 border-t border-slate-100 mt-auto space-y-4">
                                    ...
                                    <button
                                        ...
                                        className="w-full py-2.5 rounded-xl border-2 border-orange-100 text-orange-700 font-bold text-sm hover:bg-orange-50 hover:border-orange-200 transition-colors flex items-center justify-center gap-2"
                                    >
                                        عرض المواصفات كاملة
                                    </button>
                                </div>
            ...
            <SpecsModal
                ...
                titleIcon={<Watch className="w-6 h-6 text-orange-600" />}
            />
        </section>
    );
}
