                        ...
                        <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1, duration: 0.5 }} whileHover={{ y: -5 }}
                            ...
                            <div className="bg-white/95 backdrop-blur-sm rounded-[1.4rem] h-full p-6 flex flex-col relative z-10 border border-white">
                                ...
                                <button 
                                    ...
                                    className="absolute top-6 left-6 p-2 rounded-full bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors z-20"
                                >
                                ...
                                <div className="mt-auto pt-6 border-t border-slate-100 space-y-3">
                                    ...
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className="flex-1 py-2.5 rounded-xl bg-brand-dark text-white font-bold text-sm hover:bg-brand-light transition-colors flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
                                        >
                                            <ShoppingCart className="w-4 h-4" />
                                            إضافة للسلة
                                        </button>
                                        <button onClick={() => setSelectedProduct(product)} className="flex-1 py-2.5 rounded-xl border-2 border-purple-100 text-purple-700 font-bold text-sm hover:bg-purple-50 hover:border-purple-200 transition-colors flex items-center justify-center gap-2">التفاصيل</button>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
